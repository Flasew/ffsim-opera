...
public:
  ...
  int workerId, counter, guid, fromWorker, toWorker, fromGuid, toGuid;
...
public:
  ...
  virtual float update(const std::vector<OpConfig>& vec, std::vector<Task*>& tasks, std::vector<std::pair<Task*, Task*>>& edges) = 0;
...
public:
  ...
  float update(const std::vector<OpConfig>& vec, std::vector<Task*>& tasks, std::vector<std::pair<Task*, Task*>>& edges);
...
float Conv2D::update(const std::vector<OpConfig>& vec, std::vector<Task*>& tasks, std::vector<std::pair<Task*, Task*>>& edges) {
  ...
  std::queue<int> nodes_used;
  int max_intra_node = 0;
  for (int i = 0; i < NUM_NODES; i++) {
    ...
    if (cnt > 0) {
      cross_cnt ++;
      nodes_used.push(i * WORKERS_PER_NODE);
    }
  }
  ...
  int curr = nodes_used.front();
  nodes_used.pop();
  while (!nodes_used.empty()) {
    int next = nodes_used.front();
    nodes_used.pop();
...
float Pool2D::update(const std::vector<OpConfig>& vec, std::vector<Task*>& tasks, std::vector<std::pair<Task*, Task*>>& edges) {
  return 0;
}
...
float Concat::update(const std::vector<OpConfig>& vec, std::vector<Task*>& tasks, std::vector<std::pair<Task*, Task*>>& edges) {
  return 0;
}
...
float Flat::update(const std::vector<OpConfig>& vec, std::vector<Task*>& tasks, std::vector<std::pair<Task*, Task*>>& edges) {
  return 0;
}
...
float LSTM::update(const std::vector<OpConfig>& vec, std::vector<Task*>& tasks, std::vector<std::pair<Task*, Task*>>& edges)
{
  int used[NUM_WORKERS];
...
public:
  ...
  float compute(OpConfig c);
  float update(const std::vector<OpConfig>& c, std::vector<Task*>& tasks, std::vector<std::pair<Task*, Task*>>& edges);
  Rect get_tensor_shape(OpConfig c, int idx, Tensor t, bool is_input);
  //float xfer(Tensor t, OpConfig config, OpConfig preConfig);
  OpConfig get_random_config();
private:
...
float Softmax::compute(OpConfig c) {
  ...
  int idx = 0;
  for (; idx < nConfigs; idx++) {
    //printf("config_x[idx]: %d, c.dim[0]: %d, config_y[idx]: %d, c.dim[1]: %d\n", config_x[idx], c.dim[0], config_y[idx], c.dim[1]);
    if (config_x[idx] == c.dim[0] && config_y[idx] == c.dim[1])
      break;
  }
  //printf("idx: %d, nConf: %d\n", idx, nConfigs);
  assert(idx < nConfigs);
  return computeTime[idx];
};

float Softmax::update(const std::vector<OpConfig>& vec, std::vector<Task*>& tasks, std::vector<std::pair<Task*, Task*>>& edges) {
...
float Embed::update(const std::vector<OpConfig>& vec, std::vector<Task*>& tasks, std::vector<std::pair<Task*, Task*>>& edges) {
  int used[NUM_WORKERS];
...
float simulate_time(const std::map<Op*, OpConfig>& global, int iter,
    bool print = false, bool save = false)
{
  ...
  std::vector<std::pair<Task*, Task*>> taskEdges;
        ...
        for (int srcId = 0; srcId < preConfig.nParts; srcId++) {
          ...
          if (intersect(srcR, dstR) > 0) {
            ...
            if (srcT->workerId != dstT->workerId) {
              float intersectSize = intersect(srcR, dstR); 
              cost = intersectSize * LSTM_PER_NODE_LENGTH * 4 / bandwidth(srcT->workerId, dstT->workerId);
              totalDataXfer += intersectSize * LSTM_PER_NODE_LENGTH * 4;
              int comm_device_id = (srcId + 1) * MAX_NUM_WORKERS + dstId;
              Task* task = new Task(comm_device_id, cost);
              task->fromWorker = srcT->workerId;
              task->fromGuid = srcT->guid;
              task->toWorker = dstT->workerId;
              task->toGuid = dstT->guid;
              task->xferSize = intersectSize * LSTM_PER_NODE_LENGTH * 4;
              commTasks[comm_task_id++] = task;
              srcT->add_next_task(task);
              task->add_next_task(dstT);
              taskEdges.push_back(std::make_pair(srcT, task));
              taskEdges.push_back(std::make_pair(task, dstT));
            } else {
              srcT->add_next_task(dstT);
              taskEdges.push_back(std::make_pair(srcT, dstT));
            }
  ...
  if (save) {
    std::ofstream output;
    output.open("sim_taskgraph_iter" + std::to_string(iter) + ".json");
    output << "{" << std::endl;
    output << "\t\"nGPUPerNode\": " << WORKERS_PER_NODE << "," << std::endl;
    output << "\t\"nNode\": " << NUM_NODES << "," << std::endl;
    output << "\t\"tasks\": [" << std::endl;
    for (int i = 0; i < op_global_guid; i++) {
      Op * op = guidToOp[i];
      OpConfig config = global.find(op)->second;
      for (int j = 0; j < config.nParts; j++) {
        Task * currTask = tasks[i][j];
        output << "\t\t{" << std::endl; 
        output << "\t\t\t" << "\"type\": \"computation\"," << std::endl;
        output << "\t\t\t" << "\"opid\": " << std::to_string(i) << "," << std::endl;
        output << "\t\t\t" << "\"op\": \"" << op->name << "\"," << std::endl;
        ...
        output << "\t\t\t" << "\"computeTime\": " << std::to_string(currTask->computeTime) << std::endl;
      if (i != op_global_guid - 1 || j != config.nParts - 1 || comm_task_id > 0 || updateTasks.size() != 0)
        output << "\t\t}, " << std::endl;
      else 
        output << "\t\t} " << std::endl;
      }
    ...
    for (int i = 0; i < comm_task_id; i++) {
      ...
      output << "\t\t\t" << "\"toWorker\": " << std::to_string(currTask->toWorker) << "," << std::endl;
      output << "\t\t\t" << "\"fromTask\": " << std::to_string(currTask->fromGuid) << "," << std::endl;
      output << "\t\t\t" << "\"toTask\": " << std::to_string(currTask->toGuid) << "," << std::endl;
      output << "\t\t\t" << "\"xferSize\": " << std::to_string(currTask->xferSize) << "," << std::endl;
      output << "\t\t\t" << "\"workerId\": " << std::to_string(currTask->workerId) << "," << std::endl;
      output << "\t\t\t" << "\"guid\": " << std::to_string(currTask->guid) << "," << std::endl;
      output << "\t\t\t" << "\"counter\": " << std::to_string(currTask->counter) << "," << std::endl;
      output << "\t\t\t" << "\"readyTime\": " << std::to_string(currTask->readyTime) << "," << std::endl;
      output << "\t\t\t" << "\"startTime\": " << std::to_string(currTask->startTime) << "," << std::endl;
      output << "\t\t\t" << "\"computeTime\": " << std::to_string(currTask->computeTime) << std::endl;
      if (i != comm_task_id - 1 || updateTasks.size() != 0)
        output << "\t\t}, " << std::endl;
      else 
        output << "\t\t} " << std::endl;
    }
    ...
    for (int i = 0; i < taskEdges.size(); i++) {
      auto & e = taskEdges[i];
      if (i != taskEdges.size() - 1)
        output << "\t\t[" << std::to_string(e.first->guid) << ", " <<
          std::to_string(e.second->guid) << "]," << std::endl; 
      else 
        output << "\t\t[" << std::to_string(e.first->guid) << ", " <<
          std::to_string(e.second->guid) << "]" << std::endl; 
    }
...
void generate_init_config(std::map<Op*, OpConfig>& global)
{
  for (int i = 0; i < op_global_guid; i++) {
    ...
    printf("init: name: %s, c.ndims: %d\n",guidToOp[i]->name.c_str(), global[guidToOp[i]].nDims);
  }
  for (int i = 0; i < op_global_guid; i++) {
    printf("init finish: name: %s, c.ndims: %d\n",guidToOp[i]->name.c_str(), global[guidToOp[i]].nDims);
  }
}
